	...
	render: ->
		if @props.CalcWeightMode
			<g>
				...
				<rect x={if @props._xy.x != @props.__xy.x and @props._xy.y != @props.__xy.y then (Math.min(@props._xy.x, @props.__xy.x) + (Math.abs @props._xy.x - @props.__xy.x) / 2 - 23.5) else (@props._xy.x - 29.5)} 
